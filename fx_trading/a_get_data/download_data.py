...
def download_all_ninja():
    platform = Platform.NINJA_TRADER
    path = 'D:/Blackfire_Projects/Blackfire_fx_algo_trading/fx_data/{}/NT/{}/'
    with open('pairs.csv', 'r') as f:
        ...
        next(reader, None)  # skip the headers
        next(reader, None)  # skip the headers
        next(reader, None)  # skip the eaders
        # hkxhkd
        # next(reader, None)  # skip the headers
        for row in reader:
            ...
            for i in range(2017, 2023):
                for month in range(7, 13):
                    try:
                        _path = path.format(pair, str(i) + 'M' + str(month))
                        download_hist_data(year=str(i), month=str(month), platform=platform, pair=pair,
                                           time_frame=TimeFrame.TICK_DATA_LAST, output_directory=_path)
                        time.sleep(0.1)
                        download_hist_data(year=str(i), month=str(month), platform=platform, pair=pair,
                                           time_frame=TimeFrame.TICK_DATA_ASK, output_directory=_path)
                        time.sleep(0.1)
                        download_hist_data(year=str(i), month=str(month), platform=platform, pair=pair,
                                           time_frame=TimeFrame.TICK_DATA_BID, output_directory=_path)
                        time.sleep(0.1)
                    except AssertionError :
                        print(f"No values for {pair}: {i}-{month}")
                    except zipfile.BadZipFile:
                        print(f"Bad ZipFile for {pair}: {i}-{month}")
